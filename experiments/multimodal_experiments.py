...
def MULTIMODAL_TEG_PC_PI_ONLY(event_list, join_rules, conf, fname):
    ...
    plot_time_series(patient_PC, patient_BS)
    for idd in patient_PC:
        if idd in patient_BS:
            print(idd, len(patient_PC[idd]), len(patient_BS[idd]))
        else:
            print(idd, len(patient_PC[idd]))
...
def MULTIMODAL_TEG_PC_PI_NPI(event_list, join_rules, conf, fname):
    ...
    for parent_type in PI_events_grouped:
        ...
        if 'Admissions' in parent_type:
            ...
            continue
        if 'Braden' in parent_type and 'Brade Score' not in parent_type:
    ...
    braden_events = [e for e in PI_events if 'Braden Score' in e['type']]
    patient_BS = get_patient_Braden_scores(braden_events)
    plot_time_series(pi_PPC, patient_BS, npi_PPC, PI_NPI_match)
    for idd in pi_PPC:
        if idd in patient_BS:
            print(idd, len(pi_PPC[idd]), len(patient_BS[idd]))
        else:
            print(idd, len(pi_PPC[idd]))

if __name__ == "__main__":
    fname = 'output/MULTIMODAL_PI_NPI'
    '''
    fname_keys = [
    ]
    ...
    fname += '_' + '_'.join([k + '-' + str(v)
                                    ...
                                    if k in fname_keys])
    '''
    MULTIMODAL_TEG_PC_PI_NPI(EVENTS, join_rules, conf, fname)
